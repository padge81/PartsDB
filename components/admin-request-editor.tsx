...
export function AdminRequestEditor({ requestId }: { requestId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (requestResult.error) setMessage(requestResult.error.message); else {
        ...
        setAdditionalMachines((loadedRequest.machine_revision_ids ?? []).map((revisionId) => { const machine_id = (revisionResult.data ?? []).find((revision) => revision.id === revisionId)?.machine_id ?? ""; const machine = ((machineResult.data ?? []) as unknown as MachineOption[]).find((item) => item.id === machine_id); return { manufacturer_id: machine?.manufacturer?.id ?? "", machine_id, revision_id: revisionId }; }));
      }
  ...
  function updateAdditionalMachine(index: number, name: keyof MachineDraft, value: string) { setMachineRows(additionalMachines.map((row, rowIndex) => rowIndex === index ? { ...row, [name]: value, ...(name === "manufacturer_id" ? { machine_id: "", revision_id: "" } : {}), ...(name === "machine_id" ? { revision_id: "" } : {}) } : row)); }
