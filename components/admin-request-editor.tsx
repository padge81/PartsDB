...
export function AdminRequestEditor({ requestId }: { requestId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (requestResult.error) setMessage(requestResult.error.message); else setRequest(requestResult.data as EditableRequest);
  ...
  async function save(event?: FormEvent) {
    ...
    const { error } = await supabase.from("part_requests").update({ part_description: request.part_description, part_manufacturer: request.part_manufacturer || null, manufacturer_part_number: request.manufacturer_part_number || null, machine_manufacturer: request.machine_manufacturer || null, machine_model: request.machine_model || null, machine_revision: request.machine_revision || null, supply_type: request.supply_type, compatibility_tags: request.compatibility_tags ?? [], commonly_ordered_part_ids: request.commonly_ordered_part_ids ?? [], supplier_information: (request.supplier_information ?? []).filter((supplier) => supplier.supplier_id || supplier.supplier_name), notes: request.notes || null, review_notes: request.review_notes || null }).eq("id", request.id);
  ...
  return <AppShell requireAdmin>{(profile) => <main className="workspace form-workspace"><a className="back-link" href="/admin">← Back to admin requests</a>{!request ? <section className="detail-state"><div className="spinner"/><p>{message || "Loading request…"}</p></section> : <><section className="workspace-heading"><div><p className="eyebrow accent">Administrator review</p><h1>{request.part_description}</h1><p>Validate submitted information against controlled database values before approval.</p></div><span className="admin-badge"><ShieldIcon/>{request.status}</span></section><form className="record-form" onSubmit={save}>
    <section className="form-card"><div className="detail-card-heading"><h2>Part information</h2><span>Database linked</span></div><div className="form-grid"><label className="span-2">Part description *<input required value={request.part_description} onChange={(e) => field("part_description", e.target.value)}/></label><label>Part manufacturer<select value={request.part_manufacturer ?? ""} onChange={(e) => field("part_manufacturer", e.target.value)}><option value="">Select manufacturer</option>{manufacturers.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label><label>Manufacturer part number<input value={request.manufacturer_part_number ?? ""} onChange={(e) => field("manufacturer_part_number", e.target.value)}/></label><label>Supply type<select value={request.supply_type} onChange={(e) => field("supply_type", e.target.value)}><option value="unknown">Unknown</option><option value="local">Local</option><option value="dfl">DFL</option></select></label></div></section>
    <section className="form-card"><div className="detail-card-heading"><h2>Machine compatibility</h2><span>Database linked</span></div><div className="form-grid"><label className="span-2">Existing machine<select value={machineId} onChange={(e) => selectMachine(e.target.value)}><option value="">Match an existing machine</option>{machines.map((machine) => <option key={machine.id} value={machine.id}>{machine.manufacturer?.name} · {machine.model}{machine.name ? ` · ${machine.name}` : ""}</option>)}</select></label><label>Machine manufacturer<select value={request.machine_manufacturer ?? ""} onChange={(e) => field("machine_manufacturer", e.target.value)}><option value="">Select manufacturer</option>{manufacturers.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label><label>Machine model<input value={request.machine_model ?? ""} onChange={(e) => field("machine_model", e.target.value)}/></label><label>Machine revision<select value={request.machine_revision ?? ""} onChange={(e) => field("machine_revision", e.target.value)}><option value="">Select revision</option>{revisions.filter((item) => !machineId || item.machine_id === machineId).map((item) => <option key={item.id} value={item.revision}>{item.revision}</option>)}</select></label><label>Compatibility tags<select multiple value={request.compatibility_tags ?? []} onChange={(e) => setRequest({ ...request, compatibility_tags: Array.from(e.target.selectedOptions, (option) => option.value) })}>{tags.map((tag) => <option key={tag.id} value={tag.name}>{tag.name}</option>)}</select></label></div></section>
    <section className="form-card"><div className="detail-card-heading"><h2>Preferred suppliers</h2><span>Database linked · up to three</span></div><div className="supplier-form-list">{Array.from({ length: 3 }, (_, index) => request.supplier_information?.[index] ?? { preference_rank: index + 1 }).map((supplier, index) => <div className="supplier-form-row" key={index}><span>{index + 1}</span><label>Supplier<select value={supplier.supplier_id ?? ""} onChange={(e) => updateSupplier(index, "supplier_id", e.target.value)}><option value="">Not selected</option>{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Supplier part number<input value={supplier.supplier_part_number ?? ""} onChange={(e) => updateSupplier(index, "supplier_part_number", e.target.value)}/></label><label>Ordering information<input value={supplier.ordering_information ?? ""} onChange={(e) => updateSupplier(index, "ordering_information", e.target.value)}/></label></div>)}</div></section>
