...
export function AdminRequestEditor({ requestId }: { requestId: string }) {
  ...
  function selectMachine(id: string) { setMachineId(id); const machine = machines.find((item) => item.id === id); if (machine) { setMachineManufacturerId(machine.manufacturer?.id ?? ""); setRequest((current) => current ? { ...current, machine_manufacturer: machine.manufacturer?.name ?? "", machine_name: machine.name, machine_model: machine.model ?? "" } : current); } }
  ...
  function setMachineRows(rows: MachineDraft[]) { setAdditionalMachines(rows); setRequest((current) => current ? { ...current, machine_ids: rows.map((row) => row.machine_id).filter(Boolean) } : current); }
  function updateAdditionalMachine(index: number, name: keyof MachineDraft, value: string) { setMachineRows(additionalMachines.map((row, rowIndex) => rowIndex === index ? { ...row, [name]: value, ...(name === "manufacturer_id" ? { machine_id: "" } : {}) } : row)); }
  ...
  async function save(event?: FormEvent) {
    ...
    const { error } = await supabase.from("part_requests").update({ part_description: request.part_description, part_manufacturer: request.part_manufacturer || null, manufacturer_part_number: request.manufacturer_part_number || null, machine_manufacturer: request.machine_manufacturer || null, machine_name: request.machine_name || null, machine_model: request.machine_model || null, machine_ids: request.machine_ids ?? [], supply_type: request.supply_type, category_ids: request.category_ids ?? [], commonly_ordered_part_ids: request.commonly_ordered_part_ids ?? [], supplier_information: (request.supplier_information ?? []).filter((supplier) => supplier.supplier_id || supplier.supplier_name), notes: request.notes || null, review_notes: request.review_notes || null }).eq("id", request.id);
  ...
  async function approve(profile: Profile) {
    ...
    if (request.part_manufacturer) {
      const existing = await supabase.from("companies").select("id").ilike("name", request.part_manufacturer).limit(1).maybeSingle();
      if (existing.data) manufacturerId = existing.data.id;
      else { const created = await supabase.from("companies").insert({ name: request.part_manufacturer }).select("id").single(); if (created.error) { setMessage(created.error.message); setSaving(false); return; } manufacturerId = created.data.id; }
      if (manufacturerId) await supabase.from("company_roles").upsert({ company_id: manufacturerId, role: "manufacturer" }, { onConflict: "company_id,role" });
    }
    ...
    if (request.category_ids?.length) {
      const categoryResult = await supabase.from("part_categories").insert(request.category_ids.map((category_id) => ({ part_id: createdPart.data.id, category_id, created_by: profile.id })));
      if (categoryResult.error) { setMessage(`Part created, but categories could not be linked: ${categoryResult.error.message}`); setSaving(false); return; }
    }
    ...
    for (const supplier of request.supplier_information ?? []) {
      ...
      if (!supplierId && supplier.supplier_name) {
        const existing = await supabase.from("companies").select("id").ilike("name", supplier.supplier_name).limit(1).maybeSingle();
        if (existing.data) supplierId = existing.data.id;
        else { const created = await supabase.from("companies").insert({ name: supplier.supplier_name }).select("id").single(); supplierId = created.data?.id; }
      }
    ...
    if (request.machine_ids?.length) await supabase.from("part_machines").upsert(request.machine_ids.map((machine_id) => ({ part_id: createdPart.data.id, machine_id })), { onConflict: "part_id,machine_id" });

    if (request.machine_manufacturer && request.machine_name) {
      let machineManufacturerId: string | null = null;
      const existingMfr = await supabase.from("companies").select("id").ilike("name", request.machine_manufacturer).limit(1).maybeSingle();
      if (existingMfr.data) machineManufacturerId = existingMfr.data.id; else { const created = await supabase.from("companies").insert({ name: request.machine_manufacturer }).select("id").single(); machineManufacturerId = created.data?.id ?? null; }
      if (machineManufacturerId) await supabase.from("company_roles").upsert({ company_id: machineManufacturerId, role: "manufacturer" }, { onConflict: "company_id,role" });
      if (machineManufacturerId) {
        ...
        if (machineId) await supabase.from("part_machines").upsert({ part_id: createdPart.data.id, machine_id: machineId }, { onConflict: "part_id,machine_id" });
      }
    }
