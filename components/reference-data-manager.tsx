...
type Revision = { id: string; revision: string; machine_id: string; machine?: { model: string; name: string | null } | null };
...
export function ReferenceDataManager() {
  ...
  const load = useCallback(async () => {
    ...
    const [manufacturer, supplier, machine, revision, tag] = await Promise.all([
      ...
      supabase.from("machine_revisions").select("id,revision,machine_id,machine:machines(model,name)").eq("is_active", true).order("revision"),
      supabase.from("tags").select("id,name,description").order("name"),
    ]);
    setManufacturers((manufacturer.data ?? []) as Manufacturer[]); setSuppliers((supplier.data ?? []) as Supplier[]);
    setMachines((machine.data ?? []) as unknown as Machine[]); setRevisions((revision.data ?? []) as unknown as Revision[]); setTags((tag.data ?? []) as Tag[]);
  }, []);
  useEffect(() => {
  ...
  return <AppShell requireAdmin>{() => <main className="workspace reference-workspace"><a className="back-link" href="/admin">← Back to administrator portal</a><section className="workspace-heading"><div><p className="eyebrow accent">Database administration</p><h1>Reference data</h1><p>Add controlled values used by the user and approval dropdown lists.</p></div><span className="admin-badge"><ShieldIcon/>Administrator</span></section>{message && <p className="form-message success-message">{message}</p>}<div className="reference-grid">
    ...
    <ReferenceCard title="Machines" count={machines.length} items={machines.map((item) => ({ id: item.id, title: `${item.manufacturer?.name ?? ""} ${item.model}`, detail: item.name }))}><form onSubmit={(event) => add(event, "machines")}><select name="manufacturer_id" required><option value="">Machine manufacturer</option>{manufacturers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input name="model" required placeholder="Machine model"/><input name="name" placeholder="Machine name"/><button className="button primary"><PlusIcon/>Add machine</button></form></ReferenceCard>
    <ReferenceCard title="Machine revisions" count={revisions.length} items={revisions.map((item) => ({ id: item.id, title: `${item.machine?.model ?? "Machine"} · Rev ${item.revision}`, detail: item.machine?.name }))}><form onSubmit={(event) => add(event, "machine_revisions")}><select name="machine_id" required><option value="">Select machine</option>{machines.map((item) => <option key={item.id} value={item.id}>{item.manufacturer?.name} · {item.model}</option>)}</select><input name="revision" required placeholder="Revision"/><button className="button primary"><PlusIcon/>Add revision</button></form></ReferenceCard>
    <ReferenceCard title="Compatibility tags" count={tags.length} items={tags.map((item) => ({ id: item.id, title: item.name, detail: item.description }))}><form onSubmit={(event) => add(event, "tags")}><input name="name" required placeholder="Tag name"/><input name="description" placeholder="Description"/><button className="button primary"><PlusIcon/>Add tag</button></form></ReferenceCard>
  </div></main>}</AppShell>;
}
