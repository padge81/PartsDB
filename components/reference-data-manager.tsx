...
export function ReferenceDataManager() {
  ...
  const machineSearchText = machineSearch.trim().toLocaleLowerCase();
  const filteredMachines = machines.filter((machine) => !machineSearchText || [machine.name, machine.model ?? "", machine.manufacturer?.name ?? ""].some((value) => value.toLocaleLowerCase().includes(machineSearchText)));
  ...
  return <AppShell requireAdmin>{() => <main className="workspace reference-workspace"><a className="back-link" href="/admin">← Back to administrator portal</a><section className="workspace-heading"><div><p className="eyebrow accent">Database administration</p><h1>Reference data</h1><p>Add controlled values used by the user and approval dropdown lists.</p></div><span className="admin-badge"><ShieldIcon/>Administrator</span></section>{message && <p className="form-message success-message">{message}</p>}<div className="reference-grid">
    <ReferenceCard title="Companies" count={filteredCompanies.length} headingAction={<a className="button secondary compact" href="/admin/companies">Company editor</a>} items={filteredCompanies.map((item) => ({ id: item.id, title: item.name, detail: `${item.roles.map((role) => role[0].toUpperCase() + role.slice(1)).join(" · ")}${item.website_url ? ` · ${item.website_url}` : ""}` }))}><div className="reference-search"><label htmlFor="company-search">Search companies</label><input id="company-search" type="search" value={companySearch} onChange={(event) => setCompanySearch(event.target.value)} placeholder="Search by company, role or website"/></div><form className="reference-add-form" onSubmit={(event) => add(event, "companies")}><label className="reference-add-field span-2">Company name<input name="name" required placeholder="Company name"/></label><fieldset className="company-role-fields span-2"><legend>Company roles</legend><div><label><input type="checkbox" name="manufacturer"/> Manufacturer</label><label><input type="checkbox" name="supplier"/> Supplier</label><label><input type="checkbox" name="distributor"/> Distributor</label></div></fieldset><label className="reference-add-field">Supply type<select name="supply_type" defaultValue="unknown">{supplyTypes.filter((type) => type.is_active).map((type) => <option key={type.code} value={type.code}>{type.name}</option>)}</select></label><label className="reference-add-field">Default supplier<select name="default_supplier_id"><option value="">No default supplier</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label><label className="reference-add-field span-2">Website URL<input name="website_url" type="url" placeholder="https://example.com"/></label><label className="reference-add-field">Ordering information<input name="ordering_information" placeholder="Ordering information"/></label><label className="reference-add-field">Notes<input name="notes" placeholder="Notes (optional)"/></label><button className="button primary span-2"><PlusIcon/>Add company</button></form></ReferenceCard>
    ...
    <ReferenceCard title="Machines" count={filteredMachines.length} headingAction={<a className="button secondary compact" href="/admin/machines">Machine editor</a>} items={filteredMachines.map((item) => ({ id: item.id, title: item.name, detail: [item.manufacturer?.name, item.model].filter(Boolean).join(" · ") }))}><div className="reference-search"><label htmlFor="machine-search">Search machines</label><input id="machine-search" type="search" value={machineSearch} onChange={(event) => setMachineSearch(event.target.value)} placeholder="Search by machine, manufacturer or model"/></div><form className="reference-add-form" onSubmit={(event) => add(event, "machines")}><label className="reference-add-field span-2">Machine manufacturer<select name="manufacturer_id" required defaultValue=""><option value="">Select manufacturer</option>{manufacturers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="reference-add-field span-2">Machine name<input name="name" required placeholder="Machine name"/></label><label className="reference-add-field span-2">Machine model<input name="model" placeholder="Machine model (optional)"/></label><button className="button primary span-2"><PlusIcon/>Add machine</button></form></ReferenceCard>
  </div></main>}</AppShell>;
}
