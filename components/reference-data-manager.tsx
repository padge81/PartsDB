...
export function ReferenceDataManager() {
  ...
  async function updateManufacturerDefault(manufacturer: Company, supplierId: string) {
    const supabase = getSupabaseBrowserClient(); if (!supabase) return; setMessage("");
    const { error } = await supabase.from("companies").update({ default_supplier_id: supplierId || null }).eq("id", manufacturer.id);
    if (error) setMessage(error.message); else { await load(); setMessage("Default supplier updated."); }
  }

  async function updateSupplierSupplyType(supplier: Company, supplyType: string) {
    const supabase = getSupabaseBrowserClient(); if (!supabase) return; setMessage("");
    const { error } = await supabase.from("companies").update({ supply_type: supplyType }).eq("id", supplier.id);
