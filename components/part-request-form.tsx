...
export function PartRequestForm() {
  ...
  async function submit(event: FormEvent, profile: Profile) {
    ...
    const { data: request, error } = await supabase.from("part_requests").insert({ requested_by: profile.id, status: "draft", machine_manufacturer: machineManufacturer || null, machine_model: machineModel || null, machine_revision: machineRevision || null, part_description: partDescription, part_manufacturer: partManufacturer || null, manufacturer_part_number: manufacturerPartNumber || null, supplier_information: supplierInformation, supply_type: supplyType, compatibility_tags: selectedTags, commonly_ordered_part_ids: relatedIds, notes: notes || null }).select("id").single();
  ...
  return <AppShell>{(profile) => <main className="workspace form-workspace">
    ...
    <form className="record-form" onSubmit={(event) => submit(event, profile)}>
      <section className="form-card"><div className="detail-card-heading"><h2>Part information</h2><span>Required fields marked *</span></div><div className="form-grid">
        ...
        <label>Manufacturer part number<input value={manufacturerPartNumber} onChange={(e) => setManufacturerPartNumber(e.target.value)} /></label>
      ...
      <section className="form-card"><div className="detail-card-heading"><h2>Machine compatibility</h2></div><div className="form-grid">
        <label className="span-2">Existing machine<select value={machineId} onChange={(e) => selectMachine(e.target.value)}><option value="">Select an existing machine or enter details below</option>{machines.map((machine) => <option key={machine.id} value={machine.id}>{machine.manufacturer?.name} · {machine.model}{machine.name ? ` · ${machine.name}` : ""}</option>)}</select></label>
        ...
        <label>Machine model<input value={machineModel} onChange={(e) => setMachineModel(e.target.value)} /></label>
        <label>Machine revision<select value={machineRevision} onChange={(e) => setMachineRevision(e.target.value)}><option value="">Select revision</option>{revisions.filter((item) => !machineId || item.machine_id === machineId).map((item) => <option key={item.id} value={item.revision}>{item.revision}</option>)}</select></label>
      ...
      <section className="form-card"><div className="detail-card-heading"><h2>Preferred suppliers</h2><span>Up to three</span></div><div className="supplier-form-list">{supplierRows.map((row, index) => <div className="supplier-form-row" key={index}><span>{index + 1}</span><label>Supplier<select value={row.supplier_id} onChange={(e) => updateSupplier(index, "supplier_id", e.target.value)}><option value="">Not selected</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label><label>Supplier part number<input value={row.supplier_part_number} onChange={(e) => updateSupplier(index, "supplier_part_number", e.target.value)} /></label><label>Ordering information<input value={row.ordering_information} onChange={(e) => updateSupplier(index, "ordering_information", e.target.value)} /></label></div>)}</div></section>
